...
Implements the harness adapter contract (PRD.md §5.0) for Claude Code: translates
Claude Code hook payloads into the normalized `ObservabilityEvent` envelope and
POSTs them to the EOS-Observability server.
...
- `send_stage_transition.py` — explicit lifecycle stage announcer (PRD.md §3 mechanism B). Not a hook — invoke directly (e.g. via the Bash tool) at eos/ lifecycle stage boundaries.
...
2. Copy `settings.json.example` into `.claude/settings.json`, replacing `YOUR_ROLE`
   with the identity of the role/agent running in that project (see PRD.md §3
   mechanism A — this is what lets the server resolve `role` from `source_app`).
